...
class TA:
    ...
    def PPO(cls,
        ...
        EMA_fast = pd.Series(ohlc['close'].ewm(ignore_na=False,
                             min_periods=period_slow - 1,
                             span=period_fast).mean(),
                             name='EMA_fast')
        EMA_slow = pd.Series(ohlc['close'].ewm(ignore_na=False,
                             min_periods=period_slow - 1,
    ...
    def VW_MACD(cls,
        ...
        _fast = pd.Series((vp.ewm(ignore_na=False,
                           min_periods=period_fast-1, span=period_fast).mean()) /
                           (ohlcv['volume'].ewm(ignore_na=False, min_periods=period_fast-1, span=period_fast).mean()),
                           name='_fast')

        _slow = pd.Series((vp.ewm(ignore_na=False,
                           min_periods=period_slow-1, span=period_slow).mean()) /
